...
<div id="centerCol">
<h2>LoC Search for Instance</h2>

			<form method='POST' action='loc_instance.jsp'>
			...
			<c:if test="${not empty param.query}">
				...
				<c:choose>
					<c:when test="${param.mode == 'triple'}">
						<lucene:search lucenePath="/usr/local/RAID/LD4L/lucene/loc/instance"
							...
							<ol class="bulletedList">
								<lucene:searchIterator>
                                   <li><a href="loc_instance_lookup.jsp?uri=<lucene:hit label="uri" />&subject=<lucene:hit label="name" />"><lucene:hit label="name" /></a></li>
 								</lucene:searchIterator>
							</ol>
						</lucene:search>
					</c:when>
					<c:when test="${param.mode == 'literal' or empty param.mode}">
                        <lucene:search lucenePath="/usr/local/RAID/LD4L/lucene/loc/instance"
                            ...
                            <ol class="bulletedList">
                                <lucene:searchIterator>
                                    <li><a href="loc_instance_browse.jsp?uri=<lucene:hit label="uri" />&subject=<lucene:hit label="name" />"><lucene:hit label="name" /></a></li>
                                </lucene:searchIterator>
                            </ol>
                        </lucene:search>
					</c:when>
