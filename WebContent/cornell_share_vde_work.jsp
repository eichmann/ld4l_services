...
<div id="centerCol">
<h2>Cornell SHARE VDE Search for Work</h2>

			<form method='POST' action='cornell_share_vde_work.jsp'>
			...
			<c:if test="${not empty param.query}">
				...
				<c:choose>
					<c:when test="${param.mode == 'triple'}">
						<lucene:search lucenePath="/usr/local/RAID/LD4L/lucene/cornell_share_vde/work"
							...
							<ol class="bulletedList">
								<lucene:searchIterator>
                                   <li><a href="cornell_share_vde_work_lookup.jsp?uri=<lucene:hit label="uri" />&subject=<lucene:hit label="name" />"><lucene:hit label="name" /></a></li>
 								</lucene:searchIterator>
							</ol>
						</lucene:search>
					</c:when>
					<c:when test="${param.mode == 'literal' or empty param.mode}">
                        <lucene:search lucenePath="/usr/local/RAID/LD4L/lucene/cornell_share_vde/work"
                            ...
                            <ol class="bulletedList">
                                <lucene:searchIterator>
                                    <li><a href="cornell_share_vde_work_browse.jsp?uri=<lucene:hit label="uri" />&subject=<lucene:hit label="name" />"><lucene:hit label="name" /></a></li>
                                </lucene:searchIterator>
                            </ol>
                        </lucene:search>
					</c:when>
