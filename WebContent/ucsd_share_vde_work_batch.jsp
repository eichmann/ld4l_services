...
<c:set var="LuceneIndex" value="/usr/local/RAID/LD4L/lucene/ucsd_share_vde/work" />
...
<${uri}> <http://vivoweb.org/ontology/core#rank> "<lucene:hitRank/>" .
        <c:if test="${empty param.context}">
            <jsp:include page="ucsd_share_vde_work_lookup.jsp">
                <jsp:param value="${uri}" name="uri"/>
            </jsp:include>
        </c:if>
        <c:if test="${not empty param.context}">
            <jsp:include page="ucsd_share_vde_work_context.jsp">
                <jsp:param value="${uri}" name="uri"/>
            </jsp:include>
        </c:if>
