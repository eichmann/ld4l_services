...
<div id="centerCol">
<h2>LoC Search by Performance</h2>

			<form method='POST' action='loc_performance.jsp'>
			...
			<c:if test="${not empty param.query}">
				...
				<c:choose>
					<c:when test="${param.mode == 'triple'}">
						<lucene:search lucenePath="/usr/local/RAID/LD4L/lucene/loc/performance"
							...
							<ol class="bulletedList">
								<lucene:searchIterator>
                                   <li><a href="loc_performance_lookup.jsp?uri=<lucene:hit label="uri" />&subject=<lucene:hit label="name" />"><lucene:hit label="name" /></a></li>
 								</lucene:searchIterator>
							</ol>
						</lucene:search>
					</c:when>
					<c:when test="${param.mode == 'literal' or empty param.mode}">
                        <lucene:search lucenePath="/usr/local/RAID/LD4L/lucene/loc/performance"
                            ...
                            <ol class="bulletedList">
                                <lucene:searchIterator>
                                    <li><a href="loc_performance_browse.jsp?uri=<lucene:hit label="uri" />&subject=<lucene:hit label="name" />"><lucene:hit label="name" /></a></li>
                                </lucene:searchIterator>
                            </ol>
                        </lucene:search>
					</c:when>
