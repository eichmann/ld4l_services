...
<div id="centerCol">
<h2>UCSD SHARE VDE Search for Instance</h2>

			<form method='POST' action='ucsd_share_vde_instance.jsp'>
			...
			<c:if test="${not empty param.query}">
				...
				<c:choose>
					<c:when test="${param.mode == 'triple'}">
						<lucene:search lucenePath="/usr/local/RAID/LD4L/lucene/ucsd_share_vde/instance"
							...
							<ol class="bulletedList">
								<lucene:searchIterator>
                                   <li><a href="ucsd_share_vde_instance_lookup.jsp?uri=<lucene:hit label="uri" />&subject=<lucene:hit label="name" />"><lucene:hit label="name" /></a></li>
 								</lucene:searchIterator>
							</ol>
						</lucene:search>
					</c:when>
					<c:when test="${param.mode == 'literal' or empty param.mode}">
                        <lucene:search lucenePath="/usr/local/RAID/LD4L/lucene/ucsd_share_vde/instance"
                            ...
                            <ol class="bulletedList">
                                <lucene:searchIterator>
                                    <li><a href="ucsd_share_vde_instance_browse.jsp?uri=<lucene:hit label="uri" />&subject=<lucene:hit label="name" />"><lucene:hit label="name" /></a></li>
                                </lucene:searchIterator>
                            </ol>
                        </lucene:search>
					</c:when>
